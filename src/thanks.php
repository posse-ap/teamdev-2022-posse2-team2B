...
$agents = $db->prepare(sprintf("SELECT agent_name AS name, address, email, agents.tel, url, notification_email FROM agents JOIN agent_contract ON agents.id = agent_id WHERE agents.id IN (%s)", f_in_clause($inq_agent_ids)));
...
foreach ($pgdata['agents'] as $agent) {
  f_mail2agent($agent);
}
